...
public class TestSparkTransaction extends TestBaseWithCatalog {
  ...
  private Transaction newTransaction() {
    TransactionalCatalogPlugin catalog =
        (TransactionalCatalogPlugin) spark.sessionState().catalogManager().catalog(catalogName);
  ...
  private SparkTable loadTable(Transaction transaction) throws NoSuchTableException {
    ...
    Identifier sparkIdentifier = Identifier.of(tableIdent.namespace().levels(), tableIdent.name());
    return (SparkTable) transactionCatalog.loadTable(sparkIdentifier);
  }

  private void activateWrite(SparkTable table) {
